...
struct BulkExportSheet: View {
        ...
        NavigationView {
            VStack(spacing: 24) {
                ...
                VStack(spacing: 12) {
                    ...
                    Text("Download \(items.count) selected item\(items.count == 1 ? "" : "s")")
                        .font(.subheadline)
    ...
    private var dateRangeText: String {
        ...
        let dates = items.map(\.timestamp).sorted()
    ...
    private func exportData() {
        ...
        Task {
            do {
                let fileName = "QuickFlip_BulkExport_\(Date().timeIntervalSince1970).\(exportFormat.fileExtension)"
    ...
    private func generateCSV() throws -> Data {
        var csvContent = "Item Name,Category,Condition,Estimated Value,Recommended Marketplace,Best Price,Reasoning,Storage Location,Status,Timestamp\n"
        ...
        for item in items {
            ...
            let row = [
                ...
                item.listingStatus.status.rawValue,
